...
`adult_json`. Check the data. It is a list of properties
...
_capital.gain is strange, looks like missing value coded as 99999_

c) With `table` you can tabulate / count values:
...
d) Count the relationships in this data set
...
e) Count the crossing of relations and workclass

```{r, echo = params$answer}
table(adult$relationship, adult$workclass)
```

e) With `tapply` you can calculate a statistic per group
...
f) Calculate the `median` hours worked per education group
```{r}
median_hours <- 
  tapply( adult$age  # variable 
...
g) Calculate the maximum `age` per `relationship` and `workclass` combination.
