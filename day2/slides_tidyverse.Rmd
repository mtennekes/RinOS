...
output
- Therefore these _verbs_ can be chained in any particular combination
- Manipulations for: 
    - data selection
...
- They contain extra metadata usefull for dplyr
...
\code{transmute} & calculate new columns\\
...
The option \code{useNA} has three possible values:
...
With the \code{mutate} function new columns can be created

```{r}
iris <- iris |> 
  mutate(petal_length_mm = round(Petal.Length * 100),
         petal_width_mm = round(Petal.Width * 100))
```

## If else statements

With the Base R function \code{ifelse} 

```{r}
iris <- iris |> 
  mutate(petal_class = ifelse(petal_length_mm >= 500, "large", "small"))
```
...
```{r}
semi_join(d1, d2) # select records in d1 that do match in d2
```
\normalfont
...
Use `?strptime` for a list of conversion codes

## Background

### `POSIXct`

This format stores time as the number of seconds since `1970-01-01 00:00:00`.
...
\code{D} & \code{date} (locale format)\\
\code{T} & \code{date/time} (ISO8601) format.\\
\code{\_} or \code{-} & Skip column
\end{tabular}
\end{center}


## When reading does not succeed 

e.g. "money", "kg" etc.

- Read in as \code{character}
- Use \code{parse\_number} to extract the numbers.
