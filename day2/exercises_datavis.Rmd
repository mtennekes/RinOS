...
b)  Create a scatterplot with `wt` on the x-axis and `disp` on the y-axis. With `geom_point()`

```{r}
ggplot(mtcars, aes(x = wt, y = disp)) + geom_point()
```

c)  Maak the points the color "blue".
...
# ggplot(mtcars) +
#     geom_boxplot(aes(x = gear, y = mpg, color = gear))
...
a)  Read the file *UnitedNations.txt* in. Create a scatter plot with `tfr` vs life expectancy for women `lifeFemale`. Connect the aesthetic color to `region`.
...
Investigate the how `cut` and `price` interact.
...
selectie <- filter(UN, ! is.na(GDPperCapita))
ggplot(data = selectie, aes(x = GDPperCapitaGroup, y = tfr)) +
    geom_boxplot() +
    facet_grid(region ~ .)
```
